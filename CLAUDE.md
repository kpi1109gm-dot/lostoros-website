...
### 6. ヒーロー動画の扱い（2026-08-14に方針変更）

当初は「画質は絶対落としたくない」という要望から音声削除のみの無劣化運用だったが、
**4K素材（2160x3840・24秒・137MB）の受領に伴い再エンコード方式へ変更した**。理由:

- GitHubは1ファイル100MBが上限。137MBはそもそもリポジトリに置けない
- 137MBのままでは閲覧者の待ち時間が実用外

**画質は落ちていない。** 元が404x720と低解像度だったため、圧縮しても解像度が3.5倍になり
実際の見え方は大幅に向上している。CRF 23/26/28/31 を等倍で比較して差が出ないことを
確認した上で、現行とほぼ同容量に収まる CRF 31 を採用している。

- 元素材は4K・24fps・被写界深度が浅い（背景がボケた柔らかい絵）ため、
...
- 再現コマンド（PC用）:
  ...
  ```
  スマホ用は `scale=810:1440` / `-crf 30`
- **元の137MBファイルはリポジトリに入れていない**。池田さんのGoogleドライブが原本
...
`site:lostoros.net` が0件になり、Search Consoleは
**「クロール済み - インデックス未登録」**を表示していた。**サイト側の不具合ではない。**

- Googlebotのクロールは成功。noindex・robots.txt・DNS・canonical すべて正常だった
- 原因は**ドメインが新しく（取得から約2か月）、1ページのみで、外部リンクが実質ゼロ**だったこと。
  Search Consoleの「参照元ページ」は3件で、うち2件は無関係な海外・国内のスパム掲示板
- 対処: ①インデックス登録をリクエスト ②SNSプロフィールにURL記載
  ③構造化データに `sameAs` 追加 ④sitemapのlastmod更新
- 結果、**「合同会社ロストロス」で1位表示に復帰**（法人情報サイトより上）

**同じ症状が出たら、まず設定を疑うのではなくSearch Consoleの文言を見ること。**
「クロール済み - インデックス未登録」は設定ミスではなく、Googleの評価待ちを意味する。

なお**検索結果のファビコンはページ本体より遅れて反映される**（別クローラー・別スケジュール）。
出ていなくてもアイコンを作り直さないこと。取得がやり直しになって余計に遅くなる。
